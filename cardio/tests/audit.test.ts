...
describe('deterministicVerdict', () => {
  ...
  it('passes low-risk level 1 definition items without sending them to the model auditor', async () => {
    const question = {
      ...
      answer: 1,
      explanation: 'Distensibility is correct because it matches the fractional-increase clue from the source. Compliance is wrong because it refers to stored volume per pressure rise rather than fractional increase.',
  ...
  it('passes curated pressure-volume property items when deterministic validation is clean', async () => {
    const question = {
      ...
      answer: 1,
      explanation: 'Compliance is correct because veins store more blood per pressure rise than arteries do, matching the storage-capacity clue. Distensibility is wrong because it describes relative expansibility rather than total blood stored per pressure rise.',
      option_explanations: null,
