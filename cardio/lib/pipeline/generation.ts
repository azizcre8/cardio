...
- Exactly ${expectedOptionCount} options. Same comparison class — all anatomy, or all mechanisms, or all conditions; NEVER mixed (e.g. do not put an anatomical structure alongside a physiological process). NEVER append descriptor nouns or generic labels (Level, Rate, Measurement, Evaluation, Testing, Analysis, Profiling, Assessment, Concentration, Value, Reading, Mechanism, Process, Structure, Regulation, Control) to option text. All options within 1–2 words of each other in length, same grammatical shape. No two options end with the same word. No parentheticals on only the correct answer.
